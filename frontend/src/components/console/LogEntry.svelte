...
<script lang="ts">
  ...
  interface Props {
    ...
    formatConsoleType: (type: string) => string;
    formatMessage: (message: string, progressKey?: string) => string;
    /** 変換ペイン用（黄色表示を無効化） */
    isConvertPane?: boolean;
  }

  let { log, formatTime, formatConsoleType, formatMessage, isConvertPane = false }: Props = $props();
...
<div class="flex gap-2 hover:bg-gray-800 dark:hover:bg-gray-900 px-2 py-1 rounded">
  ...
  <span class="flex-1 whitespace-nowrap overflow-hidden text-ellipsis {textColorClass}">
