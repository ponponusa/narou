...
<script lang="ts">
  ...
  async function checkAozoraSettings() {
    try {
...
<div class="power-menu-container relative">
  ...
  >
    ...
    {#if serverStatus}
      <span
        class="absolute top-1 right-1 w-2 h-2 rounded-full {serverStatus
          .push_server.running
          ? 'bg-green-500'
          : 'bg-red-500'}"
      ></span>
  ...
  {#if isOpen}
    ...
    >
        ...
        <div class="space-y-2 text-sm">
          ...
          {#if serverStatus}
            <div class="flex items-center justify-between">
              ...
              <span
                class={serverStatus.push_server.running
              ...
              >
                ...
                {#if serverStatus.push_server.port}
                  <span class="text-xs text-gray-500"
                    >(Port: {serverStatus.push_server.port})</span
                  >
                {/if}
              </span>
            </div>
            ...
            <div class="flex items-center justify-between">
              ...
              <span
                class={serverStatus.queue.running
                  ? "text-green-600 dark:text-green-400"
                  : "text-gray-500 dark:text-gray-500"}
              >
                {serverStatus.queue.running
                  ? `処理中 (${serverStatus.queue.total})`
                  : "アイドル"}
              </span>
            </div>
            ...
            {#if hasAozoraEpub3 !== null}
              <div class="flex items-center justify-between">
                <span class="text-gray-600 dark:text-gray-400">Aozora:</span>
                <span
                  class={hasAozoraEpub3
                    ? "text-green-600 dark:text-green-400"
                    : "text-red-600 dark:text-red-400"}
                >
                  {hasAozoraEpub3 ? "設定済み" : "未設定"}
                </span>
              </div>
            {/if}
            ...
            <div class="flex items-center justify-between text-xs">
              ...
              <span class="text-gray-500"
                >Narou {serverStatus.version.narou}</span
              >
            </div>
          {/if}
        </div>
      ...
      {#if isLoading}
        <div
          class="px-4 py-2 text-xs text-center text-gray-500 dark:text-gray-400 border-t border-gray-200 dark:border-gray-700"
        >
          処理中...
        </div>
      {/if}
