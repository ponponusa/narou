...
describe ConverterBase do
  before do
    novelsetting = NovelSetting.create("", true, true)
  ...
  context "#insert_word_separator" do
    ...
    describe "単語単位で区切る" do
      it do
        expect(@converter.insert_word_separator("今日もいい天気ですね")).to eq \
          "今日［＃zws］もいい［＃zws］天気［＃zws］ですね［＃zws］"
      end

      it do
        expect(@converter.insert_word_separator("合計１５００円です")).to eq \
          "合計［＃zws］１５００［＃zws］円［＃zws］です［＃zws］"
      end

      it do
        expect(@converter.insert_word_separator("［＃二分アキ］「注釈内は区切らない」")).to eq \
          "［＃二分アキ］「注釈内［＃zws］は［＃zws］区切［＃zws］らない［＃zws］」"
      end

      it do
        expect(@converter.insert_word_separator("あいう・えお")).to eq \
          "あいう［＃zws］・［＃zws］えお［＃zws］"
      end

      it do
        expect(@converter.insert_word_separator("アイウ・エオ")).to eq \
          "アイウ・エオ［＃zws］"
      end

      it do
        expect(@converter.insert_word_separator("彼は――そう……言った")).to eq \
      ...
      it "ルビは除外" do
        expect(@converter.insert_word_separator("これは｜漢字《かんじ》です")).to eq \
          "これは［＃zws］｜漢字《かんじ》［＃zws］です［＃zws］"
        expect(@converter.insert_word_separator("俺の妹がこんなに｜可愛い《かわいい》わけがない")).to eq \
          "俺［＃zws］の［＃zws］妹［＃zws］がこんなに［＃zws］｜可愛い《かわいい》［＃zws］わけがない［＃zws］"
      end

      it "外字注記は連続させる" do
        expect(@converter.insert_word_separator("失われし楽園※［＃始め二重山括弧］パラダイスロスト※［＃終わり二重山括弧］!!")).to eq \
          "失［＃zws］われし［＃zws］楽園［＃zws］※［＃始め二重山括弧］［＃zws］パラダイスロスト［＃zws］※［＃終わり二重山括弧］!!"
      end

      # zws があるせいで禁則処理が正常に働かないための対応
      it "開きカッコの直後にzwsは挿入しない" do
        expect(@converter.insert_word_separator("そっちの『鈴』と一緒に入っていた")).to eq \
          "そっちの［＃zws］『鈴［＃zws］』［＃zws］と［＃zws］一緒［＃zws］に［＃zws］入［＃zws］っていた［＃zws］"
      end

      it "開きカッコの直後にzwsは挿入しない" do
        expect(@converter.insert_word_separator("（ほげ）『ふが』")).to eq \
          "（ほげ［＃zws］）『ふが［＃zws］』"
      end

      it "開きカッコの直後にzwsは挿入しない" do
        expect(@converter.insert_word_separator("［ほげ］")).to eq \
          "［ほげ［＃zws］］"
      end

      it "英語はそのまま" do
        expect(@converter.insert_word_separator("トムは言った。「This is a pen」")).to eq \
          "トム［＃zws］は［＃zws］言［＃zws］った［＃zws］。「This is a pen［＃zws］」"
      end

      it "タグの中ではzwsは挿入しない" do
        expect(@converter.insert_word_separator("click <a href=\"http://google.com\">link</a>")).to eq \
    ...
    describe "文字単位で区切る" do
      it do
        expect(@converter.insert_char_separator("今日もいい天気")).to eq \
          "今［＃zws］日［＃zws］も［＃zws］い［＃zws］い［＃zws］天［＃zws］気［＃zws］"
      end

      it "連続させないとまずいものは連続させる" do
        expect(@converter.insert_char_separator("――何ということでしょう……。")).to eq \
          "――［＃zws］何［＃zws］と［＃zws］い［＃zws］う［＃zws］こ［＃zws］と［＃zws］で［＃zws］し［＃zws］ょ［＃zws］う［＃zws］……［＃zws］。［＃zws］"
      end

      it "外字注記は連続させる" do
        expect(@converter.insert_char_separator("失われし楽園※［＃始め二重山括弧］パラダイスロスト※［＃終わり二重山括弧］!!")).to eq \
  ...
  describe "#to_ruby" do
    ...
    describe "｜を使ったルビ位置の指定" do
      it "《 》基本" do
        expect(to_ruby("｜山田太郎《やまだたろう》", "｜山田太郎", "やまだたろう", ["《", "》"])).to eq "｜山田太郎《やまだたろう》"
      end
      it "（ ）基本" do
        expect(to_ruby("｜山田太郎（やまだたろう）", "｜山田太郎", "やまだたろう", ["（", "）"])).to eq "｜山田太郎《やまだたろう》"
      end
    ...
    describe "ルビの自動対応" do
      it "（ ）自動" do
        expect(to_ruby("HP（ヒットポイント）", "HP", "ヒットポイント", ["（", "）"])).to eq "｜HP《ヒットポイント》"
      end
      it "《 》自動" do
        expect(to_ruby("ＭＰ《マジックポイント》", "ＭＰ", "マジックポイント", ["《", "》"])).to eq "｜ＭＰ《マジックポイント》"
      end
      it "（ ）漢字と認識されない1" do
        expect(to_ruby("久々（ひさびさ）", "久々", "久々（ひさびさ）", ["（", "）"])).to eq "久々（ひさびさ）"
      end
      it "（ ）漢字と認識されない2" do
        expect(to_ruby("二〇八（ふたまるはち）", "二〇八", "ふたまるはち", ["（", "）"])).to eq "二〇｜八《ふたまるはち》"
      end
      it "（ ）ルビ化されない" do
        expect(to_ruby("山田太郎（山田太郎）", "山田太郎", "山田太郎", ["（", "）"])).to eq "山田太郎（山田太郎）"
      end
      it "《 》ルビ化されない" do
        expect(to_ruby("山田太郎《山田太郎》", "山田太郎", "山田太郎", ["《", "》"])).to eq "山田太郎《山田太郎》"
      end
    ...
    describe "スペースを含む場合の特殊処理" do
      it "（ ）の中にスペースを一つ含む場合" do
        expect(to_ruby("山田　太郎（やまだ　たろう）", "山田　太郎", "やまだ　たろう", ["（", "）"])).to eq "｜山田《やまだ》　｜太郎《たろう》"
      end
      it "（ ）ルビにスペースがない場合" do
        expect(to_ruby("山田　太郎（やまだたろう）", "山田 太郎", "やまだたろう", ["（", "）"])).to eq "山田 ｜太郎《やまだたろう》"
      end
      it "（ ）文字列にスペースがない場合" do
        expect(to_ruby("山田太郎（やまだ　たろう）", "山田太郎", "やまだ　たろう", ["（", "）"])).to eq "｜山田太郎《やまだ　たろう》"
      end
      it "（ ）アルファベットもルビがふられる対象" do
        expect(to_ruby("ＮＥＷ山田　太郎（やまだ　たろう）", "ＮＥＷ山田　太郎", "やまだ　たろう", ["（", "）"])).to eq "｜ＮＥＷ山田《やまだ》　｜太郎《たろう》"
      end
      it "《 》文字列は分割されない" do
        expect(to_ruby("｜山田　太郎《やまだ　たろう》", "｜山田　太郎", "やまだ　たろう", ["（", "）"])).to eq "｜山田　太郎《やまだ　たろう》"
      end
    end
  end
end
