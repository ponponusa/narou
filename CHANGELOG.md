...
- **ドキュメント構成の整理**
  - README をプロジェクト紹介と利用ユーザー向けの内容に再構成し、インストール手順を追加。
  - 開発者向け情報（環境構築・ビルド・API/フロントエンド開発・Windows 同期）を `docs/development.md` に集約。
  - 未レビューの開発ドキュメントをリポジトリの公開対象から除外。
- **リリースノートのインストールコマンドを修正**
  - 生成されるリリースノートの `gem install` コマンドが実際のアセットファイル名と一致するように修正（Linux/macOS 版 gem にプラットフォームサフィックスは付きません）。
- **フロントエンド依存関係の更新**
  - Astro 5.18.2 / Svelte 5.56.8 / Tailwind CSS 4.3.3 / Playwright 1.62.1 / Prettier 3.9.6 など、現行メジャーバージョン範囲内の最新に更新。
  - Prettier 3.9 の整形規則変更に合わせて既存ソース 4 ファイルを再整形（動作変更なし）。

## 3.1.6 (2026-07-29)

### 🐛 バグ修正

- **ハーメルンのダウンロードが 403 Forbidden で失敗する問題を改善 (#123)**
  - Firefox 互換の User-Agent と HTML 取得用のナビゲーションヘッダーを使用。
  - R18 作品の分離先 `h.syosetu.org` と同ドメインへのリダイレクトに対応。ただし、環境によって
    Cloudflare のブラウザ確認に拒否された場合は、自動回避せず原因と再試行方法を表示。
  - 2026年7月の目次 HTML 変更に対応し、従来形式との互換性も維持。
...
- **Web UI サーバへのアクセスログ出力に対応 (#120)**
  - `--verbose` または `--log-file` 指定時に、高頻度ポーリング API
    (`/api/v2/system/status`, `/api/v2/tasks`) のノイズを除外したアクセスログを出力。
...
- **調査ログ.txt が空になる問題を修正**
  - メッセージがない場合は調査ログファイルを作成しないよう修正
  - ファイル保存時のエラーハンドリングを強化
  - ディレクトリ自動作成とUTF-8エンコーディング対応を追加

- **コンソールパネルの stdout/stderr 表示混在問題を修正**
  - `stdout2` を `stderr` と誤表示していた問題を修正
  - 並列変換用出力は `parallel` として表示するよう変更
  - 不要な黄色テキスト表示を削除
