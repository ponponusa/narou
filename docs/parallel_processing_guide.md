...
| 設定 | 実行時間 | CPU使用率 | User time | 改善率 |
|------|---------|----------|-----------|--------|
| シーケンシャル | 171.9秒 | 99% | 169.5秒 | - |
| 並列（2コア、エピソード単位） | 90.6秒 | 189% | 170.6秒 | 47.3% |
| **並列（2コア、チャンク最適化）** | **90.5秒** | 187% | **168.4秒** | **47.3%** |
| 並列（4コア）* | 約60秒 | 350% | - | 約65% |
...
| チャンクサイズ | プロセス起動回数 | 実行時間 | User time | 評価 |
|--------------|----------------|---------|-----------|------|
| 500 | 8回 | 91.4秒 | 168.3秒 | 良好 |
| **1000** | **4回** | **90.5秒** | **168.4秒** | **最適** ✅ |
| 1948 (旧デフォルト) | 2回 | 92.9秒 | 171.2秒 | やや遅い |
| 3895 (全体) | 1回 | 157.3秒 | 156.5秒 | シーケンシャル化 |

**結論**: チャンクサイズ1000が最適（プロセス起動オーバーヘッドと並列効率の最良バランス）
