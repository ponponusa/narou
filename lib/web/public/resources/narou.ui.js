  ...
  function initializeComponents() {
    ...
    function buildTagFilter(currentFilter, newTagFilter, mode) {
      ...
      if (mode === 'or') {
        ...
        // 最後のタグ検索を探す
        var lastTagMatch = currentFilter.match(/(^|\s)([-^]?)tag:([^\s]+)(\s|$)/g);
        if (lastTagMatch) {
          var lastTag = lastTagMatch[lastTagMatch.length - 1].trim();
          var lastTagExclude = lastTag.match(/^([-^]?)tag:/)[1];
          var lastTagContent = lastTag.replace(/^[-^]?tag:/, '');
          
          // 同じ除外フラグの場合、パイプで繋げる
          if (lastTagExclude === newExclude) {
            var updatedTag = (lastTagExclude || '') + 'tag:' + lastTagContent + '|' + newTag;
            return currentFilter.replace(/(^|\s)([-^]?)tag:([^\s]+)(\s*)$/, '$1' + updatedTag + '$4');
          }
        }
      }
