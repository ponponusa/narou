...
$(function() {
  ...
  */
    ...
    buttons: [
      {
        ...
        // ColVisの exclude 相当:
        // このセレクタに含まれる列だけ列表示/非表示の対象にする
        // => 除外したい列には列定義側で className: 'title' 等を付けておく
  ...
  // （存在しないと trigger('click') で動かないことがあるため）
  ...
  // colvisボタンDOMを取得
  var $colvisButton = $btnContainer.find('button.buttons-colvis');
  ...
  $("#action-view-setting").on(click_event_name, function(e) {
    ...
    if (touchable_device) {
      ...
      pos = Narou.get_event_position(e); // 既存ヘルパーで {x, y} が返る想定
    }

    slideNavbar.slide(); // 既存動作

    // colvisメニューを開かせる
    $colvisButton.trigger('click');

    // DataTables Buttons が生成したドロップダウン(コレクション)を取得
    var $collection = $('.dt-button-collection').last();

    // 旧ColVisの colvis._fnCollectionShow() 相当:
    // メニューをクリック位置にフローティング表示
    $collection.css({
      position: 'absolute',
      left: pos.x,
      top:  pos.y,
      display: 'block',
      zIndex: 2000 // 必要に応じて調整（Bootstrapの.navbarやモーダルとの重なり用）
    });
  });
