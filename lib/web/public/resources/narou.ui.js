...
$(function() {
  ...
  function initializeDataTable() {
    ...
    initComplete: function(_settings, _json) {
      ...
      console.log("Action initialized:", action);
  ...
  function initializeComponents() {
    ...
    $(window).on('hashchange', function(e) {
      console.log("=== hashchangeイベント検出 ===");
      console.log("現在のハッシュ:", location.hash);
      console.log("イベント詳細:", e);
      
      // フィルタフィールドも更新
      var filterString = Narou.Location.getFilterString();
      console.log("取得したフィルタ文字列:", filterString);
      $("#myFilter").val(filterString || '');
      
      // DataTablesのURLを更新してリロード（処理中表示なし）
      var newUrl = createAjaxUrl();
      console.log("生成されたAjax URL:", newUrl);
      
      // テーブルが存在するかチェック
      if (!table) {
        console.error("テーブルが初期化されていません！");
        return;
      }
      
      console.log("テーブルリロードを実行中...");
      // フィルタリング時は処理中表示を無効化
      table.ajax.url(newUrl).load(function(json) {
        console.log("テーブルリロード完了:", json);
        console.log("受信データ数:", json?.data?.length || 0);
      }, false);
    });
    ...
    $(document).on('click', '.tag[data-tag]:not(.tag-reset)', function(e) {
      ...
      console.log("取得したタグ名:", tagName);
      
      if (tagName && tagName.trim()) {
        var filterString = 'tag:' + tagName;
        console.log("設定するフィルタ文字列:", filterString);
        
        $("#myFilter").val(filterString);
        console.log("フィルタフィールド更新完了");
        
        console.log("setHashWithEventを呼び出し中...");
        Narou.Location.setHashWithEvent(filterString);
        
        storage.set("filter_string", filterString).save();
        console.log("ローカルストレージ更新完了");
      } else {
        console.warn("タグ名が空または無効:", tagName);
      }
    });
    ...
    $(document).on('click', 'td:contains("作者名") a, .author-link, [data-author]', function(e) {
      ...
      if (authorName && authorName.trim()) {
        ...
        console.log("作者名クリック:", authorName);
