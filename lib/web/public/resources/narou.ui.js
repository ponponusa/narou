...
$(function() {
  ...
  function resettingTableState(skipDraw) {
    ...
    // ページを跨いだ選択状態を復元
    if (typeof action !== 'undefined' && action && action.updateCurrentPageSelection) {
      action.updateCurrentPageSelection();
    }
  ...
  function initializeComponents() {
    ...
    // タグクリック時のフィルタリング（イベントデリゲーション）
    // 修飾キーによる検索モード：
    // 何もなし: AND検索, Shift: -AND検索, Ctrl: OR検索, Shift+Ctrl: -OR検索
    $(document).on('mousedown', '.tag[data-tag]:not(.tag-reset)', function(e) {
      // mousedownで早期にキャプチャして、ショートカット処理との競合を避ける
      if (e.which === 1) { // 左クリックのみ
        ...
        // タグ要素にフラグを設定
        $(this).data('tag-modifier-keys', tagModifierKeys);
      }
    });
    
    $(document).on('click', '.tag[data-tag]:not(.tag-reset)', function(e) {
      e.preventDefault();
      e.stopPropagation();
      e.stopImmediatePropagation(); // より強力なイベント停止
      
      var modifierData = $(this).data('tag-modifier-keys');
      if (!modifierData) {
        // フォールバック: mousedownが実行されなかった場合
        modifierData = {
          ctrl: e.ctrlKey,
          shift: e.shiftKey,
          tagName: $(this).attr('data-tag')
      ...
      var tagName = modifierData.tagName;
      if (tagName && tagName.trim()) {
        ...
        // 修飾キーを確認して適切な検索構文を構築
        if (modifierData.ctrl && modifierData.shift) {
          // Shift + Ctrl: - OR検索
          newTagFilter = '-tag:' + tagName;
          finalFilter = buildOrFilter(currentFilter, newTagFilter);
        } else if (modifierData.ctrl) {
          // Ctrl: OR検索
          newTagFilter = 'tag:' + tagName;
          finalFilter = buildOrFilter(currentFilter, newTagFilter);
        } else if (modifierData.shift) {
          // Shift: - AND検索
          finalFilter = '-tag:' + tagName;
        } else {
          // 何もなし: AND検索
          finalFilter = 'tag:' + tagName;
        }
      ...
      // データをクリア
      $(this).removeData('tag-modifier-keys');
    });
    
    // OR検索のフィルタ構築ヘルパー関数
    function buildOrFilter(currentFilter, newTagFilter) {
      if (!currentFilter) {
        return newTagFilter;
      }
      
      // 既存のフィルタがタグ検索かチェック
      var tagMatch = currentFilter.match(/^([-^]?)tag:(.+)$/i);
      if (tagMatch) {
        var existingExclude = tagMatch[1];
        var existingTags = tagMatch[2];
        var newExclude = newTagFilter.startsWith('-') ? '-' : '';
        var newTag = newTagFilter.replace(/^-?tag:/, '');
        
        // 同じ除外フラグの場合、パイプで繋げる
        if (existingExclude === newExclude) {
          return (existingExclude || '') + 'tag:' + existingTags + '|' + newTag;
        }
      }
      
      // 異なるタイプのフィルタまたは複雑な場合は、スペース区切りで繋げる
...
Mac スタイル：<br>メニューが画面外にはみ出そうとしたら、メニューがはみ出ないよう上にずれる
  ...
  var register_badge_number_of_selecting = function(table) {
    table.on("changeselect", function() {
      ...
      var selected_count;
      if (action && action.selectedIds && action.selectedIds.size > 0) {
        selected_count = action.selectedIds.size;
      } else {
