                ...
                main: {
                  ...
                  callback: function () {
                    ...
                    $.post("/api/edit_tag", {
                      ...
                      .done(function () {
                        console.log("タグ編集API成功: フォールバックとしてテーブルリロードを実行");

                        // タグ追加がある場合はより長い待機時間
                        var fallbackDelay = hasAdditions ? 1500 : 500;
                        console.log("フォールバック待機時間:", fallbackDelay + "ms");

                        // フォールバック処理：WebSocketが失敗した場合のための確実なリロード
                        setTimeout(function () {
                          console.log("フォールバックリロード実行中...");
                          if (hasAdditions) {
                            console.log("タグ追加があるため強制リロードを実行");
                            Narou.tableReload(true); // 強制リロード
                          } else {
                            Narou.tableReload();
                          }
                        }, fallbackDelay);
