...
var Narou = (function () {
  ...
  $.extend(Action.prototype, {
    ...
    update: function () {
      ...
      // selectedIds の状態を確実にチェック
      var hasSelectedIds = this.selectedIds && this.selectedIds.size > 0;
      console.log("[DEBUG] hasSelectedIds:", hasSelectedIds);

      if (hasSelectedIds) {
        var selectedArray = Array.from(this.selectedIds);
        console.log("[DEBUG] Using cross-page selection from selectedIds");
        console.log("[DEBUG] selectedArray.length:", selectedArray.length);
        console.log("[DEBUG] selectedArray contents:", selectedArray);
        console.log("[DEBUG] Sending POST to /api/update with ids:", selectedArray);

        $.post("/api/update", { "ids": selectedArray })
          .done(function (data) {
...
<label><input type="checkbox" id="update-gl-and-update-modified" <% if (updateModified) { %>checked<% } %>> 確認後にmodifiedタグが付いた小説をUpdate</label>
    ...
    updateForce: function () {
      ...
      // ページ跨ぎ選択がある場合は、selectedIdsから直接取得（DataTables制限を回避）
      if (this.selectedIds.size > 0) {
        var selectedArray = Array.from(this.selectedIds);
        console.log("[DEBUG] Using cross-page selection from selectedIds:", selectedArray.length, "items");
        console.log("[DEBUG] Selected IDs:", selectedArray);
        $.post("/api/update", { "ids": selectedArray, "force": "true" })
          .done(function (data) {
