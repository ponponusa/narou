        ...
        $.post("/api/taginfo.json", {
          ...
          self._createEditorFieldWithData(ids, taginfo, sortState, function (field) {
            ...
            setTimeout(function () {
              ...
              console.log("[DEBUG] Looking for checkboxes in bootbox-body:", $(".bootbox-body input[type=checkbox]").length);
              self.setDefaultTagState($(".bootbox-body"));
            }, 500);
          });
        }).fail(function () {
          // ソート状態取得に失敗した場合は従来通りの処理
    ...
    _createEditorFieldWithData: function (ids, taginfo, sortState, callback) {
      ...
      _.each(taginfo, function (info) {
        ...
        var label = $('<label><input type="checkbox" data-tagname="' + info.tag.replace(/"/g, "&quot;") +
          '" data-default-checkstate=' + state + '> ' + info.html + '&nbsp;&nbsp;</label>');
        field.append(label);
      });

      field.append('<div><label for="new-tag">新しいタグを追加：</label> <input type="text" id="new-tag" name="new-tag" class="form-control"></div>');
      callback(field.html());
    },
