...
class NovelConverter
  ...
  module TextProcessor
    ...
    def subtitles_to_sections(subtitles, html)
      ...
      # 並列処理数（環境変数で上書き可能、デフォルト: 2）
      parallel_count = (ENV['NAROU_PARALLEL_THREADS'] || 2).to_i
      
      # Ractorベース並列処理（デフォルト有効、100エピソード以上で自動適用）
      if subtitles.size > 100
        stream_io.puts "Using Ractor-based parallel processing (#{subtitles.size} episodes)" if ENV['NAROU_DEBUG']
